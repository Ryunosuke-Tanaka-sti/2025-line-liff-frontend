...
export const RouletteContent = ({ stringArray }: SelectContentProps) => {
  ...
  return (
    <>
      <div className="flex w-full max-w-2xl flex-col gap-6 rounded-xl bg-white/10 p-8 shadow-2xl backdrop-blur-lg">
        <h1 className=" text-center text-3xl font-bold text-white">
          ランダム雑談選択
        </h1>

        <div className=" flex min-h-40 items-center justify-center rounded-lg bg-white/20 p-6">
          {isLoading ? (
            <div className="flex flex-col items-center gap-4">
              <div className="size-12 animate-spin rounded-full border-y-2 border-white"></div>
              <p className=" text-white">選択中...</p>
            </div>
          ) : selectedString ? (
